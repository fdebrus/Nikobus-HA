...
class NikobusEventListener:
    ...
    def _extract_frames(self, raw: str) -> list[str]:
        ...
        if "\r" not in self._frame_buffer: return []
    ...
    def validate_crc(self, message: str) -> bool:
        """FIXED: Refined Nikobus CRC and Length validation."""
        if message.count("$") > 1:
        ...
        try:
            ...
            # FIXED: Nikobus length field is total chars after '$' + 1
            if len(message) != expected_total - 1:
                _LOGGER.error("Length mismatch: %s vs actual %d (frame: %s)", total_len_hex, len(message)-1, message)
                # Note: If frames continue to fail, you can temporarily return True 
                # to debug the payload, but keep this check for stability.
                return False
