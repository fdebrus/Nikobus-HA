...
class NikobusConfig:
    ...
    async def load_json_data(self, file_name: str, data_type: str) -> dict | None:
        ...
        try:
            ...
            raise HomeAssistantError(
                f"Failed to decode JSON in {data_type} file: {e}"
            ) from e
    ...
    async def write_json_data(self, file_name: str, data_type: str, data: dict) -> None:
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to write {data_type.capitalize()} data to file {file_name}: {e}"
            )
            raise HomeAssistantError(
                f"Failed to write {data_type.capitalize()} data to file {file_name}: {e}"
            ) from e
            ...
            raise HomeAssistantError(
                f"Failed to serialize {data_type} data to JSON: {e}"
            ) from e
        except Exception as e:
            _LOGGER.error(
                f"Unexpected error writing {data_type} data to file {file_name}: {e}"
            )
            raise HomeAssistantError(
                f"Unexpected error writing {data_type} data to file {file_name}: {e}"
            ) from e
