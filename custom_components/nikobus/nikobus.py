...
class Nikobus:
    ...
    async def setOutputState(self, address, channel, value):
        ...
        _LOGGER.debug("Nikobus Group: %s", group_number)
    ...
    async def _send_command_answer(self, writer, reader, command):
        if asyncio.get_running_loop() is not None:
            answer = await _async_send_command_and_get_answer(writer, reader, command)
        else:
            async with async_timeout.timeout(10):
                answer = await asyncio.new_event_loop().run_in_executor(None, lambda: asyncio.run(_async_send_command_and_get_answer(writer, reader, command)))
        return answer
