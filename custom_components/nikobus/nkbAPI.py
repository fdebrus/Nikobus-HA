...
class NikobusAPI:
    ...
    async def turn_on_switch(
        ...
        channel_info = self._coordinator.dict_module_data["switch_module"][address][
            "channels"
        ][channel - 1]
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to turn on switch at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def turn_off_switch(
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to turn off switch at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def turn_on_light(
        ...
        try:
            ...
            if current_brightness == 0:
                channel_info = self._coordinator.dict_module_data["dimmer_module"][
                    address
                ]["channels"][channel - 1]
                led_on = channel_info.get("led_on")
                if led_on:
            ...
            _LOGGER.error(
                f"Failed to turn on light at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def turn_off_light(
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to turn off light at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def stop_cover(
        ...
        channel_data = self._coordinator.dict_module_data["roller_module"][address][
            "channels"
        ][channel - 1]
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to stop cover at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def open_cover(
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to open cover at address {address}, channel {channel}: {e}"
            )
            raise
    ...
    async def close_cover(
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to close cover at address {address}, channel {channel}: {e}"
            )
            raise
