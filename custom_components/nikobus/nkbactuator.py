...
class NikobusActuator:
    ...
    def __init__(self, coordinator, dict_button_data, dict_module_data):
    ...
    def _start_press_task(self, address: str):
        ...
        self._press_task = self._coordinator.hass.async_create_task(self._wait_for_release(address))
    ...
    def _start_timer_tasks(self, address: str):
        ...
        for duration in [1, 2, 3]:
            task = self._coordinator.hass.async_create_task(
        ...
        try:
            ...
            while True:
                ...
                if time_diff >= self._debounce_time_ms:
                    ...
                    self._coordinator.hass.async_create_task(self.button_discovery(address))
                    _LOGGER.debug(f"Firing timer event nikobus_button_released for address: {address}")
                    
                    self._coordinator.hass.bus.async_fire("nikobus_button_released", {"address": address})
    ...
    def _fire_duration_event(self, address: str, press_duration: float):
        ...
        event_map = {
            SHORT_PRESS: "nikobus_short_button_pressed",
            MEDIUM_PRESS: "nikobus_button_pressed_1",
            LONG_PRESS: "nikobus_button_pressed_2",
        }

        for duration, event in event_map.items():
            if press_duration < duration:
                _LOGGER.debug(f"Firing event {event} for address: {address}")
                self._coordinator.hass.bus.async_fire(event, {"address": address})
                return

        # Longest press event
        _LOGGER.debug(f"Firing event nikobus_button_pressed_3 for address: {address}")
        self._coordinator.hass.bus.async_fire("nikobus_button_pressed_3", {"address": address})
        _LOGGER.debug(f"Firing event nikobus_long_button_pressed for address: {address}")
        self._coordinator.hass.bus.async_fire("nikobus_long_button_pressed", {"address": address})
    ...
    async def process_button_modules(self, button_data: dict, button_address: str) -> None:
        ...
        for impacted_module_info in button_data.get("impacted_module", []):
            ...
            if not impacted_module_address or not impacted_group:
                ...
                continue
            try:
                await asyncio.sleep(DIMMER_DELAY if impacted_module_address in self._dict_module_data.get("dimmer_module", {}) else REFRESH_DELAY)
                
                value = await self._coordinator.nikobus_command_handler.get_output_state(impacted_module_address, impacted_group)
                if value is not None:
                    self._coordinator.set_bytearray_group_state(impacted_module_address, impacted_group, value)
                    self._coordinator.hass.bus.async_fire("nikobus_button_pressed", {
