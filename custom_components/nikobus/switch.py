...
async def async_setup_entry(hass: HomeAssistant, entry, async_add_entities) -> bool:
    ...
    switch_modules_addresses = data['switch_modules_addresses']

    entities = []

    for switch_module in switch_modules_addresses:
        description = switch_module.get("description")
        model = switch_module.get("model")
        address = switch_module.get("address")
        nbrOutputs = switch_module.get("nbrOutputs")
    
        if None not in (description, model, address, nbrOutputs):
            for i in range(1, nbrOutputs + 1):
                entities.append(NikobusSwitchEntity(hass, dataservice, description, model, address, i))
        else:
            _LOGGER.error("Incomplete data for switch module: %s", switch_module)
