...
- **Repair issues and the legacy-button flow now name the bridge buttons exactly as they are labelled**, in English, French and Dutch. A few texts still carried the names those actions had in earlier releases — the reprogrammed-module warning asked for *Scan all module links* when the button reads *2. Load Existing Installation*, and the French *no PC-Link known* error named a button that no longer exists under that wording. Eighteen references realigned across the three languages; the number prefixes (*1.*, *2.*) are included so the text points at one specific button. Reported by a user following the Dutch wording.
...
- **Dimmer link labels read as the Nikobus software shows them** (`nikobus-connect 0.37.2`): the M11 / M12 preset in a button's link attributes is the level on the dim controller's 1–10 V scale, `1,0 V … 10,0 V`, where it used to say `1% … 10%`; the M01–M03 dimming-speed setting carries the vendor's wording. Roller *Stop* links (M04) no longer show an operating time. Visible after the next *Scan all module links*.
- A feedback frame pushed by a Feedback Module is now logged at DEBUG (`Feedback frame for module 4707 group 2 — outputs 000000000000`), so the push path can be seen working instead of inferred from the raw bus frames.
- **A start-up where nothing answers on the port is said out loud** (`nikobus-connect 0.37.2`): the library now follows the handshake with a presence probe that the PC-Link or a feedback module used as gateway acknowledges, and any other Nikobus frame relayed meanwhile counts as well. When the port opens but nothing answers — wrong port, PC-Link unpowered, serial handle left dead by the Nikobus PC software — the integration still starts, but a Repair issue names the port and what to check, instead of every command timing out silently. Installations that use a PC-Logic as gateway are not blocked: if the probe stays unanswered there while everything works, the warning can be ignored and clears on the next connection that is answered.
- *Load Existing Installation* now greys out while *Verify*, *Backup* or *Sync PC-Link clock* runs, like the other bridge buttons; it kept its own discovery-only availability check and stayed pressable during a verify.
