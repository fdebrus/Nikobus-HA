...
- **The start-up presence probe no longer warns falsely on a quiet bus** (`nikobus-connect 0.38.0`). The previous probe only earned an acknowledgement that the PC-Link holds back until it has something else to send, so installations without Feedback Module traffic logged *no Nikobus device answered* on every start. The probe is now the `#A` identity broadcast, which the PC-Link answers itself within tens of milliseconds.
