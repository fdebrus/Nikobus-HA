...
- **Start-up fails fast when nothing answers on the port** (`nikobus-connect 0.37.2`): the library now follows the handshake with a presence probe the PC-Link (or a feedback module used as gateway) must acknowledge. A wrong port, an unpowered PC-Link or a serial handle left dead by the Nikobus PC software used to "connect" and then time out on every command; they now fail the setup with a message saying what to check, and Home Assistant retries by itself.
